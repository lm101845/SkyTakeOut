...
public class AutoFillAspect {
    /**
     * 切入点：mapper包下的所有方法，且方法上有@AutoFill注解
     */
    ...
    public void autoFill(JoinPoint joinPoint){
        ...
        //获取到当前被拦截的方法上额数据库操作类型
        MethodSignature signature = (MethodSignature)joinPoint.getSignature();  //方法签名对象
        AutoFill autoFill = signature.getMethod().getAnnotation(AutoFill.class);//获取到当前被拦截的方法上的注解对象
        OperationType operationType = autoFill.value();                         //获取到当前被拦截的方法上的注解对象的value值
        //获取到当前被拦截的方法上的参数——实体对象
        Object[] args = joinPoint.getArgs();  //约定：第一个参数是实体对象
